...
// ✅ แปลง HEIC → JPG ผ่าน CloudConvert
export async function heicToJpgCloudConvert(file: File): Promise<File> {
  ...
  while (status !== "finished") {
    ...
    const exportTask = Object.values(pollJob.data.tasks).find(
      (t) => t.name === "exportFile" && t.result?.files?.[0]?.url
    );
  ...
  return new File([jpgBlob], file.name.replace(/\.heic$/i, ".jpg"), {
    type: "image/jpeg",
  });
}
...
// ✅ แปลง HEIC ถ้าจำเป็น แล้วอัปโหลด Supabase จบในฟังก์ชันเดียว
export async function convertAndUploadToSupabase(
  ...
  // ตรวจสอบนามสกุลไฟล์ ถ้าเป็น HEIC ให้แปลงก่อน
  if (/\.heic$/i.test(file.name) || file.type === "image/heic") {
    console.log("🔄 แปลงไฟล์ HEIC เป็น JPG ก่อนอัปโหลด...");
    fileToUpload = await heicToJpgCloudConvert(file);
  } else {
    console.log("✅ ไฟล์ไม่ใช่ HEIC อัปโหลดตรงไป Supabase...");
  }

  // อัปโหลดไฟล์ไป Supabase Storage
  const publicUrl = await uploadAboutImage(fileToUpload, uploadPath);
  console.log("✅ Upload สำเร็จ! Public URL:", publicUrl);
